...
class KbpSentenceSpec extends FlatSpec {
  ...
  "KbpSentences" should "deserialize then serialize to their original string" in {
    ...
    val testSrcs = sentFiles map { f => getClass.getClassLoader.getResource(f) } map { res => scala.io.Source.fromURL(res, "UTF8") }
  ...
  "KbpSentences" should "have offsets that correctly key into source doc" in {
    ...
    corpora.zip(rawFiles) foreach { case (corpus, sample) => 
        val source = io.Source.fromURL(getClass.getClassLoader.getResource(sample))
